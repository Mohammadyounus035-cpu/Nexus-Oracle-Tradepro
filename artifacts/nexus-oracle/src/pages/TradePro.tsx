...
export default function TradePro() {
  const { prices, portfolio, trades, executeOrder, selectedSymbol, setSelectedSymbol, history } = useMarket();
      ...
      <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        <div className="col-span-3 flex flex-col gap-4">
          <GlassCard className="flex-1 flex flex-col overflow-hidden">
            <div className="p-3 border-b border-border/30 text-xs font-mono text-primary tracking-widest">WATCHLIST</div>
              ...
              {stocksList.map(quote => {
                ...
                return (
                  <div 
                    ...
                    className={`p-3 border-b border-border/10 cursor-pointer flex items-center justify-between transition-colors ${isSelected ? 'bg-primary/10' : 'hover:bg-primary/5'}`}
                    data-testid={`row-${quote.symbol}`}
                  >
                    <div>
                      <div className="font-bold text-foreground font-sans">{quote.symbol}</div>
